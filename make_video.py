...
if not os.path.exists(WAV_FILE):
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-i", MUSIC_FILE, "-c:a", "pcm_s16le", "-f", "wav", WAV_FILE],
        capture_output=True,
        check=True,
    )
else:
...
for j in range(N - 1):
    ...
    trans = transition_overrides.get(transition_pair, xfade_types[j % len(xfade_types)])
    offset = (j + 1) * (D - T)
